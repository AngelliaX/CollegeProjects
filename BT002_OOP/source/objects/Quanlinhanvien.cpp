...
int Quanlinhanvien::quickSortBySalary(int left, int right, bool(*mode)( double, double)){
...
int Quanlinhanvien::checkCapicity(){
...
int Quanlinhanvien::updateObject(int Pos){
    Pos -= 1;
    Nhanvien Nv;
    Nhanvien:cin >> Nv;
    array[Pos] = Nv;
...
int Quanlinhanvien::deleteObject(int Pos){
...
int Quanlinhanvien::addNewObject(Nhanvien Nv, int Pos){
    int n = currentEmptyPos+1;
    currentEmptyPos += 1;
    Pos -= 1;
...
std::ostream &operator<<(std::ostream &out, const Quanlinhanvien &Ql)
{
    out << blue +"Ten nguoi quan li: "+reset+ Ql.tennguoiquanli +", " +blue+"So luong n.vien: "+reset+ to_string(Ql.currentEmptyPos);
    return out;
}